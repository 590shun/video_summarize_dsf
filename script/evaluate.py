...
res_sum_rel_to_avg = '|   relative to avg.human |{:^12.4}|{:^12.4}|{:^12.4}|{:^17.4}|'
...
def eval_f1(pred_summary, gt_summary):
    ...
    f1 = list(map(lambda y_true: f1_score(y_true, pred_summary), gt_summary))
    #print(f1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('resbase', type=str, default='', help='path to resulting .npy files')
    parser.add_argument('--dname', '-d', type=str, default='summe', help='dataset name(summe)')
    ...
    hum_min, hum_avg, hum_max = eval_human_summary(dataset_name, gt_base)
    
    print('**********---------------------------- Result ----------------------------**********')
    print(header)
    
    for k in res:
        print(entry.format(k, hum_min[k], hum_avg[k], hum_max[k], res[k]))
    
    score_mean = [sum(r.values()) /len(r) for r in [hum_min, hum_avg, hum_max, res]]


    print('|' + '|' * 82 +'|')
    print(res_sum_mean.format(*score_mean))
    print(res_sum_rel_to_avg.format(*map(lambda x: x / score_mean[1], score_mean)))
